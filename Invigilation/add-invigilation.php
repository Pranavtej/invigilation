...
if(isset($_POST['submit']))
{
                                    ...
                                    $ename= $_POST['times'];
                                    $q="select etype,time,from_date,to_date from exam_data where e_name='$ename'";
...
<table class="table table-hover table-center mb-0 datatable">
    <thead>
                                ...
                                echo '<th><button  class="btn btn-primary" name="'.$dt->format("l").'"><a href="staff-invigilation.php?day='.$dt->format("l").'&time='.$tt.'&date='.$dt->format("d-m-y \n").'&ecode='.$ename.'&etype='.$etype.'">'.$dt->format("d-m-y \n").'</a></button>
                                           </th>';
                                    }
                                ?>
                                </tr>
...
<li class="nav-item"><a class="nav-link  active" href="#samp" data-bs-toggle="tab">Read me</a></li>
                                ...
                                foreach ($period as $dt) {
                                        ...
                                        if($dt->format('l') != "Sunday")
                                        {
                                            ...
                                            echo'<li class="nav-item"><a class="nav-link " href="#tab'.$i.'" data-bs-toggle="tab">'.$dt->format("d-m-y \n").'</a></li>';
...
<div class="tab-pane show active" id="samp"> Show Active </div>
                            <?php
                             $i=1;
                                foreach ($period as $dt) {
                                        ...
                                        if($dt->format('l') != "Sunday")
                                        {
                                            ...
                                            <?php
                                                ...
                                                // if(!($row = mysqli_fetch_assoc($run)) echo'NO satff Available';
                                                // else
                                                 while($row = mysqli_fetch_assoc($run))
                                                     {
                                                        ...
                                                        //echo nl2br("$s \n");
                                                    ?>
                                                    <input type="checkbox" name="brands[]" value="<?php echo $s?>"/> <?php echo $s;
                                                     }
